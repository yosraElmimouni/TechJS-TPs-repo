...
import { BookData, Status } from "./types.js";
import { getStats, Stats } from "./api.js";
...
function createBookElement(book: Book): HTMLDivElement {
  ...
  const deleteBtn = div.querySelector(".deleteBtn") as HTMLButtonElement;
  ...
  const updateBtn = div.querySelector(".updateBtn") as HTMLButtonElement;
  const pagesReadInput = div.querySelector(`#pagesRead-${book.id}`) as HTMLInputElement;
  ...
  updateBtn.addEventListener("click", async () => {
    ...
    try {
      await book.updatePagesRead(newPagesRead);
      location.reload();
    } catch (error) {
